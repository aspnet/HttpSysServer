...
namespace Microsoft.AspNetCore.Server.HttpSys
{
    internal class ResponseBody : Stream
    {
        ...
        private unsafe void FlushInternal(bool endOfRequest, ArraySegment<byte> data = new ArraySegment<byte>())
        {
            ...
            uint statusCode = 0;
            HttpNativeStructs.HTTP_DATA_CHUNK[] dataChunks;
            ...
            try
            {
                ...
                {
                    fixed (HttpNativeStructs.HTTP_DATA_CHUNK* pDataChunks = dataChunks)
                    {
        ...
        private List<GCHandle> PinDataBuffers(bool endOfRequest, ArraySegment<byte> data, out HttpNativeStructs.HTTP_DATA_CHUNK[] dataChunks)
        {
            ...
            {
                dataChunks = new HttpNativeStructs.HTTP_DATA_CHUNK[1];
            ...
            {
                // No data
                dataChunks = new HttpNativeStructs.HTTP_DATA_CHUNK[0];
                return pins;
            }
            ...
            dataChunks = new HttpNativeStructs.HTTP_DATA_CHUNK[chunkCount];
        ...
        private static void SetDataChunk(HttpNativeStructs.HTTP_DATA_CHUNK[] chunks, ref int chunkIndex, List<GCHandle> pins, ArraySegment<byte> buffer)
        {
        ...
        private unsafe Task FlushInternalAsync(ArraySegment<byte> data, CancellationToken cancellationToken)
        {
            ...
            if ((flags & HttpNativeStructs.HTTP_FLAGS.HTTP_SEND_RESPONSE_FLAG_MORE_DATA) == 0)
            {
        ...
        private HttpNativeStructs.HTTP_FLAGS ComputeLeftToWrite(long writeCount, bool endOfRequest = false)
        {
            ...
            {
                flags |= HttpNativeStructs.HTTP_FLAGS.HTTP_SEND_RESPONSE_FLAG_MORE_DATA;
            }
