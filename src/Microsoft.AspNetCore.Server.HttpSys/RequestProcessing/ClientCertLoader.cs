...
namespace Microsoft.AspNetCore.Server.HttpSys
{
    ...
    internal unsafe sealed class ClientCertLoader : IAsyncResult, IDisposable
    {
        ...
        private static readonly int RequestChannelBindStatusSize =
            Marshal.SizeOf<HttpNativeStructs.HTTP_REQUEST_CHANNEL_BIND_STATUS>();

        private SafeNativeOverlapped _overlapped;
        private byte[] _backingBuffer;
        private HttpNativeStructs.HTTP_SSL_CLIENT_CERT_INFO* _memoryBlob;
        private uint _size;
        ...
        private HttpNativeStructs.HTTP_SSL_CLIENT_CERT_INFO* RequestBlob
        {
            get
            {
                return _memoryBlob;
        ...
        private void Reset(uint size)
        {
            ...
            _memoryBlob = (HttpNativeStructs.HTTP_SSL_CLIENT_CERT_INFO*)Marshal.UnsafeAddrOfPinnedArrayElement(_backingBuffer, 0);
        }
            ...
            do
            {
                ...
                uint statusCode =
                    HttpApi.HttpReceiveClientCertificate(
                        ...
                        (uint)HttpNativeStructs.HTTP_FLAGS.NONE,
                        RequestBlob,
                        size,
                        &bytesReceived,
                        NativeOverlapped);
                ...
                {
                    HttpNativeStructs.HTTP_SSL_CLIENT_CERT_INFO* pClientCertInfo = RequestBlob;
        ...
        private static unsafe void IOCompleted(ClientCertLoader asyncResult, uint errorCode, uint numBytes)
        {
            ...
            try
            {
                if (errorCode == UnsafeNclNativeMethods.ErrorCodes.ERROR_MORE_DATA)
                {
                    ...
                    HttpNativeStructs.HTTP_SSL_CLIENT_CERT_INFO* pClientCertInfo = asyncResult.RequestBlob;
                ...
                {
                    HttpNativeStructs.HTTP_SSL_CLIENT_CERT_INFO* pClientCertInfo = asyncResult._memoryBlob;
            ...
            do
            {
                ...
                fixed (byte* blobPtr = blob)
                {
                    ...
                    statusCode = HttpApi.HttpReceiveClientCertificate(
                        requestQueue.Handle,
                        connectionId,
                        (uint)HttpNativeStructs.HTTP_FLAGS.HTTP_RECEIVE_SECURE_CHANNEL_TOKEN,
                        blobPtr,
                        (uint)size,
                        &bytesReceived,
        ...
        private static int GetTokenOffsetFromBlob(IntPtr blob)
        {
            ...
            IntPtr tokenPointer = Marshal.ReadIntPtr(blob, (int)Marshal.OffsetOf<HttpNativeStructs.HTTP_REQUEST_CHANNEL_BIND_STATUS>("ChannelToken"));
        ...
        {
            Debug.Assert(blob != IntPtr.Zero);
            return Marshal.ReadInt32(blob, (int)Marshal.OffsetOf<HttpNativeStructs.HTTP_REQUEST_CHANNEL_BIND_STATUS>("ChannelTokenSize"));
        }
    }
}
